...
export interface Garden {
  ...
  emotion: EmotionVector;
  dominantEmotion?: string; // <---- add this
}
...
// add this (or replace the old DirectoryStats) in src/utils/db.ts
interface DirectoryStats {
  fileCount: number;
  totalSize: number;
  avgFileAge: number;     // milliseconds
  lastModified: number;   // timestamp ms
  depth: number;
  hasHiddenFiles: boolean;
  hasTempFiles: boolean;
  codeFileRatio: number;  // 0..1
...
export class GardenDB {
  ...
  private load() {
    try {
      const json = fs.readJsonSync(this.filePath, { throws: false }) as DBSchema | null;
      ...
      this.data = { gardens: [] };
      try { fs.writeJsonSync(this.filePath, this.data, { spaces: 2 }); } catch (_) {}
    }
  }
  ...
  getAllGardens(): Garden[] {
    ...
    return JSON.parse(JSON.stringify(this.data.gardens)) as Garden[];
  }
...
private scanDirectory(dirPath: string): DirectoryStats {
  ...
  const traverse = (current: string, depth = 0) => {
    ...
    for (const name of items) {
      ...
      if (st.isDirectory()) {
        ...
        if (depth < 6) traverse(child, depth + 1);
        ...
        if (CODE_EXTENSIONS.has(ext)) codeFiles++;
        if (st.size > 1024 * 1024) largeFileCount++;
      }
    }
  };
...
const estimatedHealth = Math.max(
    ...
    Math.min(
        ...
        100
            - Math.max(
                0,
                Math.floor(scan.avgFileAge / (1000 * 60 * 60 * 24)) - 30
            )
            - (scan.hasHiddenFiles ? 5 : 0)
            - (scan.hasTempFiles ? 10 : 0)
            - Math.floor(scan.largeFileCount * 2)
  ...
  updateGarden(id: string, patch: Partial<Garden>): Garden | null {
    const index = this.data.gardens.findIndex(g => g.id === id);
    if (index === -1) return null;

    const existing = this.data.gardens[index];
    const updated: Garden = { ...existing, ...patch };
  ...
  waterGarden(id: string): Garden | undefined {
